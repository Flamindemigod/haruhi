    ...
    if (anime.mediaListEntry) {
        ...
        if (anime.mediaListEntry.startedAt.year) {
          setMediaStartDate(
            new Date().setFullYear(
              anime.mediaListEntry.startedAt.year,
              anime.mediaListEntry.startedAt.month - 1,
              anime.mediaListEntry.startedAt.day
            )
          );
        } else {
      ...
      if (anime.mediaListEntry.completedAt) {
        if (anime.mediaListEntry.completedAt.year) {
          setMediaEndDate(
            new Date().setFullYear(
              anime.mediaListEntry.completedAt.year,
              anime.mediaListEntry.completedAt.month - 1,
              anime.mediaListEntry.completedAt.day
            )
          );
        } else {
      ...
      >
        <Box className="relative w-full">
          <Box
            ...
            sx={{ aspectRatio: "21 / 6", maxHeight: "20rem" }}
          >
            <Image
          ...
          <form
            className="flex flex-wrap p-8 justify-center gap-4"
            onSubmit={() => {
          ...
          >
            ...
            <FormControl sx={{ width: "10rem" }}>
              <TextField
                ...
                onChange={(e) => {
                  setMediaRewatches(e.target.value);
                }}
              />
            </FormControl>
            <FormControl sx={{ width: "10rem" }}>
