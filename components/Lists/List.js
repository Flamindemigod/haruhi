...
const List = ({ status }) => {
  ...
  return (
    <Box
      ...
      sx={{ width: "100%", marginInline: "auto" }}
    >
      <div className="flex font-semibold h-32 sm:h-16 w-full gap-4 justify-center items-center bg-offWhite-600">
        ...
        <div
          className="flex gap-2 items-center text-xs sm:text-lg w-7/12 cursor-pointer"
          onClick={(event) => {
            onClickSort(
              sort,
              setSort,
        ...
        >
          Title{" "}
          {renderSwitch(sort, "MEDIA_TITLE_ROMAJI_DESC", "MEDIA_TITLE_ROMAJI")}
        </div>
        <div
          onClick={(event) => {
            onClickSort(sort, setSort, "SCORE_DESC", "SCORE");
          }}
          className="flex gap-2 items-center cursor-pointer text-xs sm:text-lg w-1/12"
        >
          Score {renderSwitch(sort, "SCORE_DESC", "SCORE")}
        </div>
        <div className="flex gap-2 items-center text-xs sm:text-lg w-1/12">
          Type
        </div>
        <div
          onClick={(event) => {
            onClickSort(sort, setSort, "PROGRESS_DESC", "PROGRESS");
          }}
          className="flex gap-2 items-center  cursor-pointer text-xs sm:text-lg w-1/12"
          style={{ minWidth: "3rem" }}
        >
          Progress {renderSwitch(sort, "PROGRESS_DESC", "PROGRESS")}
        </div>
      </div>
      {animeList.map((anime) => (
        <Link key={anime.id} href={`/anime/${anime.id}`}>
          <div className="flex h-32 sm:h-16 w-full gap-4 justify-center items-center cursor-pointer">
            ...
            <div className="text-xs overflow-hidden text-ellipsis sm:text-sm w-7/12">
              {anime.title.userPreferred}
            </div>
            <div className="text-xs sm:text-sm w-1/12">
              {anime.mediaListEntry.score}
            </div>
            <div className="text-xs sm:text-sm w-1/12">{anime.format}</div>
            <div
              className="text-xs sm:text-sm w-1/12"
              style={{ minWidth: "3rem" }}
            >
              {anime.mediaListEntry ? anime.mediaListEntry.progress : 0}{" "}
