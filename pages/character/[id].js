...
const Character = ({ character }) => {
  ...
  useEffect(() => {
    ...
    if (onList && user.userAuth) {
      setMedia(
        character.media.nodes.filter((media) => {
    ...
    <>
      ...
      <div className="p-4">
        <div className="flex justify-center sm:px-16  sm:flex-row flex-col">
          ...
          <div className="flex flex-col p-4 justify-center">
            ...
            <div className="px-8 pb-8 text-lg">
              {character.name.alternative.map((name) => `| ${name} |`)}
            </div>
            {character.bloodType ? (
            ...
            character.dateOfBirth.day ? (
              <div className="text-md px-8">
                <strong>Date of Birth:</strong> {character.dateOfBirth.day}{" "}
                {months[character.dateOfBirth.month - 1]}{" "}
                {character.dateOfBirth.year}
              </div>
            ) : (
              <></>
        ...
        <div className="w-full flex justify-center">
          <Box
            ref={grid}
            className={`grid justify-center gap-4 w-max ${
              hasHover
...
export async function getServerSideProps({ params, req }) {
  ...
  while (hasNextPage) {
    const characterData = await makeQuery(
      query,
      variables,
    ...
    if (characterData.data.Character.media.nodes.length) {
      ...
      for (const media in characterData.data.Character.media.nodes) {
        characterData.data.Character.media.nodes[media] = getAiring(
          characterData.data.Character.media.nodes[media]
        );
      }
      accumalatedMedia = [
        ...accumalatedMedia,
        ...characterData.data.Character.media.nodes,
      ];
    } else {
      hasNextPage = false;
    }
