  ...
  return (
    <>
      {!!el && (
        ...
        >
          <Image
            src={el.coverImage.medium}
            fill
            sizes="10vw"
            className={"object-cover"}
            alt={el.title.userPreferred}
          />
        </Link>
      )}
    </>
  ...
  const {
    ...
    queryFn: async ({ pageParam = 1 }) => {
      const res = await fetch(
        `https://haruhi.flamindemigod.com/api/getStaff?id=${
          params.slug
    ...
    getNextPageParam: (lastPage, pages) => {
      return lastPage.data.Staff.characterMedia.pageInfo.hasNextPage
        ? lastPage.data.Staff.characterMedia.pageInfo.currentPage + 1
        : false;
    },
  });
      ...
      <div className="p-4">
        <div className="flex justify-center sm:px-16 sm:flex-row flex-col">
          <Image
            ...
            src={data?.pages[0].data.Staff.image.large}
            alt={`Character ${data?.pages[0].data.Staff.name.userPreferred}`}
          />
          <div className="flex flex-col p-4 justify-center">
            <div className="px-8 text-3xl">
              {data?.pages[0].data.Staff.name.userPreferred}
            </div>
            <div className="px-8 pb-8 text-lg">
              {data?.pages[0].data.Staff.name.alternative.map(
                (name: string) => `| ${name} |`
              )}
            </div>
            {data?.pages[0].data.Staff.bloodType ? (
            ...
            {data?.pages[0].data.Staff.gender ? (
              <div className="text-md px-8">
                <strong>Gender:</strong> {data?.pages[0].data.Staff.gender}
              </div>
            ) : (
              <></>
            )}
            {data?.pages[0].data.Staff.dateOfBirth.year ||
            data?.pages[0].data.Staff.dateOfBirth.month ||
            data?.pages[0].data.Staff.dateOfBirth.day ? (
              <div className="text-md px-8">
                <strong>Date of Birth:</strong>{" "}
                {data?.pages[0].data.Staff.dateOfBirth.day}{" "}
                {months[data?.pages[0].data.Staff.dateOfBirth.month - 1]}{" "}
                {data?.pages[0].data.Staff.dateOfBirth.year}
              </div>
            ) : (
              <></>
            )}
            {data?.pages[0].data.Staff.age ? (
              <div className="text-md px-8 ">
                <strong>Age:</strong> {data?.pages[0].data.Staff.age}
              </div>
            ) : (
              <></>
            )}
